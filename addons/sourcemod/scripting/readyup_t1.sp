...
public void SurvFreezeChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	...
	if (inReadyUp)
	{
		ReturnTeamToSaferoom(L4D_TEAM_SURVIVOR);
		SetTeamFrozen(L4D_TEAM_SURVIVOR, readySurvFreeze);
	}
}
...
public void PlayerTeam_Event(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (team == L4D_TEAM_UNASSIGNED && oldteam != L4D_TEAM_SPECTATOR) // Player disconnecting
	{
...
public Action Timer_PlayerTeam(Handle timer, DataPack dp)
{
	...
	if (client == GetClientOfUserId(userid))
	{
		if (inLiveCountdown)
		{
			...
			if (team != oldteam)
			{
				if (oldteam != L4D_TEAM_UNASSIGNED || team != L4D_TEAM_SPECTATOR)
				{
		...
		if (IsClientInGame(client) && !IsFakeClient(client))
		{
			...
			if (IsPlayer(client))
			{
				if (isPlayerReady[client])
				{
					...
					GetClientTeam(client) == L4D_TEAM_SURVIVOR ? StrCat(survivorBuffer, sizeof(survivorBuffer), nameBuf) : StrCat(infectedBuffer, sizeof(infectedBuffer), nameBuf);
				}
				else 
				{
...
void InitiateLive(bool real = true)
{
	...
	SetTeamFrozen(L4D_TEAM_SURVIVOR, false);
...
bool CheckFullReady()
{
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client) && isPlayerReady[client])
		{
			...
			{
				case L4D_TEAM_SURVIVOR: survReadyCount++;
				case L4D_TEAM_INFECTED: infReadyCount++;
			}
		}
	}
	
	int survLimit = GetTeamHumanCount(L4D_TEAM_SURVIVOR);
	int zombLimit = GetTeamHumanCount(L4D_TEAM_INFECTED);
	...
	if (l4d_ready_unbalanced_start.BoolValue)
	{
		...
		return (iBaseline <= GetTeamHumanCount(L4D_TEAM_SURVIVOR) <= survReadyCount)
			&& (iBaseline <= GetTeamHumanCount(L4D_TEAM_INFECTED) <= infReadyCount);
	}
	else
	{
...
void CancelFullReady(int client, disruptType type)
{
	if (readyCountdownTimer != null)
	{
		SetTeamFrozen(L4D_TEAM_SURVIVOR, GetConVarBool(l4d_ready_survivor_freeze));
...
{
	if (GetClientTeam(client) == L4D_TEAM_SURVIVOR && !blockSecretSpam[client])
	{
...
stock bool IsAnyPlayerLoading()
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientConnected(i) && (!IsClientInGame(i) || GetClientTeam(i) == L4D_TEAM_UNASSIGNED))
		{
			return true;
		}
	}
...
{
	SetEntityMoveType(client, freeze ? MOVETYPE_NONE : (GetClientTeam(client) == L4D_TEAM_SPECTATOR ? MOVETYPE_NOCLIP : MOVETYPE_WALK));
}
